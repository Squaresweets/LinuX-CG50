...
// Just default RAM amount is 64MB, but we have 16kb lol
uint32_t ram_amt = 16*1024;
...
int main(void)
{
    Bdisp_AllClr_VRAM(); //calculator stuff
	...
	int single_step = 0;
	int dtb_ptr = 0;
	const char * image_file_name = 0; //Todo
	...
	if( !ram_image )
	{
		//Error: could not allocate system image.
		return -4;
	}
...
restart:
	{
		...
		fseek( f, 0, SEEK_END ); //get the length
		long flen = ftell( f );
		fseek( f, 0, SEEK_SET );
		if( flen > ram_amt )
		{
